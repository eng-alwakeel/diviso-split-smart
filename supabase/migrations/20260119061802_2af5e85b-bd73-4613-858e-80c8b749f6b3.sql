-- Fix get_group_balance function to correctly calculate net_balance with settlements
-- The correct formula: net_balance = (amount_paid - amount_owed) + settlements_out - settlements_in
-- Because: settlements_out = money you paid to settle your debt (reduces your negative balance)
--          settlements_in = money you received as settlement (reduces your positive balance)
...
BEGIN
  ...
  -- Only count confirmed settlements (exclude pending and disputed)
  sin AS (
  ...
  SELECT
    ...
    -- Fixed formula: settlements_out increases balance (debt payment), settlements_in decreases balance (receiving payment)
