 ...
 * Supports both guest and registered identities.
 */
export function resolveHomeMode(profile: UserDataProfile): HomeModeResult {
  ...
  if (profile.identity_type === 'guest') {
    ...
    if (profile.guest_temporary_groups_count > 0 && profile.guest_temporary_expenses_count > 0) {
      ...
      resolution_reason = `Guest has ${profile.guest_temporary_groups_count} temp group(s) with ${profile.guest_temporary_expenses_count} expense(s) → share_ready_mode (register to share)`;
    } else if (profile.stale_days >= THRESHOLDS.STALE_DAYS && profile.has_in_progress_data) {
      current_home_mode = HOME_MODES.RE_ENGAGEMENT;
      resolution_reason = `Guest inactive for ${profile.stale_days} days with temp data → re_engagement_mode`;
    } else if (profile.has_in_progress_data) {
      current_home_mode = HOME_MODES.IN_PROGRESS;
      resolution_reason = `Guest has temp drafts/plans in progress → in_progress_mode`;
    } else {
      current_home_mode = HOME_MODES.FIRST_ENTRY;
      resolution_reason = `Guest with no meaningful temp usage → first_entry_mode`;
    }

    // Guest overlays
    if (profile.entered_via_invite_link) {
      active_overlays.push(OVERLAYS.INVITE_PRIORITY);
      active_overlays.push(OVERLAYS.AUTH_REQUIRED_GATE);
      resolution_reason += `. Guest entered via invite link → invite_priority + auth_required_gate overlays activated (registration required to join group)`;
    }
  } else {
    // --- Registered user resolution (existing logic) ---
    ...
    // auth_required_gate: never activated for registered users
  }
