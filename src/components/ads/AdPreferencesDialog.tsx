      ...
      <DialogContent className="max-w-md max-h-[80vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <Switch
              id="show_ads"
              checked={localPreferences?.show_ads || false}
              onCheckedChange={(checked) =>
                setLocalPreferences(prev => prev ? { ...prev, show_ads: checked } : null)
              }
            />
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <Switch
              id="personalized_ads"
              checked={localPreferences?.personalized_ads || false}
              onCheckedChange={(checked) =>
                setLocalPreferences(prev => prev ? { ...prev, personalized_ads: checked } : null)
              }
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <Slider
              value={[localPreferences?.max_ads_per_session || 3]}
              onValueChange={([value]) =>
                setLocalPreferences(prev => prev ? { ...prev, max_ads_per_session: value } : null)
              }
              max={10}
              min={1}
              step={1}
                ...
                return (
                  <div key={category.id} className="flex items-center justify-between p-2 rounded-lg border">
                    ...
                    <div className="flex items-center gap-2">
                      {isPreferred && (
                        <Badge variant="secondary" className="text-xs">مفضل</Badge>
                      )}
                      {isBlocked && (
                        <Badge variant="destructive" className="text-xs">محجوب</Badge>
                      )}
                      ...
                      <div className="flex items-center gap-1">
                        <Button
                          ...
                          onClick={() => togglePreferred(category.id)}
                          disabled={isBlocked}
                        >
                          أفضل
                        </Button>
