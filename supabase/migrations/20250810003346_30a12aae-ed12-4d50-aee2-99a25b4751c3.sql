...
-- Read-only for authenticated users; manage via service role
DO $$ BEGIN
  IF NOT EXISTS (
...
DO $$ BEGIN
  ...
  END IF;
END $$;
-- Fill payer_id from existing created_by when null
UPDATE public.expenses SET payer_id = created_by WHERE payer_id IS NULL;
-- FKs
DO $$ BEGIN
  -- profiles FK for payer
  IF NOT EXISTS (
...
-- RLS: allow payer to update their expense
DO $$ BEGIN
  IF NOT EXISTS (
  ...
  END IF;
END $$;
-- Ensure updated_at trigger on expenses
DO $$ BEGIN
...
-- Add optional category budget, amount_limit, starts_on
DO $$ BEGIN
  IF NOT EXISTS (
...
DO $$ BEGIN
  ...
  END IF;
END $$;
-- Backfill starts_on from start_date when null
UPDATE public.budgets SET starts_on = start_date WHERE starts_on IS NULL;
-- Indexes
CREATE INDEX IF NOT EXISTS idx_budgets_category_id ON public.budgets(category_id);

-- 7) Budget limit check trigger (prevent over-limit)
...
BEGIN
  -- Only enforce on INSERT/UPDATE (DELETE reduces spend)
  IF TG_OP IN ('INSERT','UPDATE') THEN
    -- Check matching category budget first (if any), then group-level
    ...
    -- For each matching budget, compute total including this change and compare to limit
    FOR 
      SELECT b.id, COALESCE(b.amount_limit, b.total_amount) AS limit_amt,
             COALESCE(b.starts_on, b.start_date) AS s, b.end_date AS e,
             b.category_id AS bc
      ...
      WHERE b.group_id = v_group
        ...
        AND (v_date >= COALESCE(b.starts_on, b.start_date)
             AND (b.end_date IS NULL OR v_date <= b.end_date))
    LOOP
      -- Sum spent in the period for same scope
      IF (SELECT COALESCE(SUM(e.amount),0) FROM public.expenses e
          WHERE e.group_id = v_group
            AND (CASE WHEN bc IS NULL THEN true ELSE e.category_id = bc END)
            AND (e.spent_at::date >= s AND (e.spent_at::date <= COALESCE(e, e)))
         ) + v_amount > limit_amt THEN
        v_over := true;
        EXIT;
      END IF;
    END LOOP;
...
-- Policies: owner or admins manage invites
DO $$ BEGIN
  IF NOT EXISTS (
...
-- 16) Helpful constraint/trigger: ensure payer_id defaults to created_by if null on insert
