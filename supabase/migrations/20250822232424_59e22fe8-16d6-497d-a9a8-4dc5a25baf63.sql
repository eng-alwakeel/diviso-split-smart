-- إصلاح مشكلة constraint على أرقام الهواتف
-- أولاً، سنقوم بتنظيف البيانات الموجودة

-- المرحلة الثانية: تحسينات الأمان (بدون القيود التي تسبب مشاكل)
...
DROP POLICY IF EXISTS "Users can read their own security logs" ON public.security_logs;
...
BEGIN
  ...
  IF v_user_email IS NULL THEN
    -- تسجيل محاولة تحقق فاشلة (إذا كان الجدول موجوداً)
  ...
  -- تسجيل محاولة التحقق (إذا كان الجدول موجوداً)
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'security_logs') THEN
...
BEGIN
  ...
  RETURN phone_input ~ '^(\+966|966|0)?5[0-9]{8}$' 
    ...
    OR length(phone_input) >= 10; -- مرونة إضافية للأرقام الأخرى
END;
$$;
