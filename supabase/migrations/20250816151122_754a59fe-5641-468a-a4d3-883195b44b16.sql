...
SELECT supabase_replicate_table('public.expenses');
SELECT supabase_replicate_table('public.settlements');
SELECT supabase_replicate_table('public.expense_splits');

-- Add expense rejections to realtime
ALTER TABLE public.expense_rejections REPLICA IDENTITY FULL;
SELECT supabase_replicate_table('public.expense_rejections');